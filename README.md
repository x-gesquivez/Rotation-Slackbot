...
Automatically selects 2 people for **Service Desk** and assigns 2 **Operations** tasks each to the remaining members. Runs twice daily: a **next-day preview at 5:30 PM** that locks in tomorrow's assignments, and a **morning announcement at 9:00 AM** that re-posts the locked-in result.
...
Or run locally:
```bash
# Trigger the morning selection
FORCE_RUN=1 python bot.py

# Trigger the next-day preview
FORCE_PREVIEW=1 python bot.py

# Morning run ignoring preview (re-randomize)
FORCE_RUN=1 FORCE_RESELECT=1 python bot.py
```
...
1. **Exclude unavailable people**: Anyone excluded for the target day (e.g., `MONDAY_EXCLUSIONS`) is removed from the rotation entirely
...
- **Scheduling**: Runs at 9:00 AM and 5:30 PM Pacific time Monday-Friday, year-round
- **How it works**: Railway cron fires at minutes 0 and 30 at hours 0, 1, 16, and 17 UTC (Mon-Sat). Code guards (`should_run_now` and `should_run_preview`) ensure only the correct local-time triggers execute — extra fires are ignored
...
Railway's Hobby plan is sufficient for this use case. The cron fires multiple times daily to cover DST variations, but only 2 executions actually run (5:30 PM preview + 9:00 AM morning), using minimal resources.
